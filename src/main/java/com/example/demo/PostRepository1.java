...
//        session ->
//
// session.createQuery(query).setFirstResult(offset).setMaxResults(limit).getResultList());
//  }
//
//  public Uni<Post> findById(UUID id) {
//    Objects.requireNonNull(id, "id can not be null");
//    return this.sessionFactory
//        .withSession(session -> session.find(Post.class, id))
//        .onItem()
//        .ifNull()
...
//    if (post.getId() == null) {
//      return this.sessionFactory.withSession(
//          session -> session.persist(post).chain(session::flush).replaceWith(post));
//    } else {
//      return this.sessionFactory.withSession(
//          session -> session.merge(post).onItem().call(session::flush));
//    }
//  }
//
