...
public class DataInitializer implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) throws Exception {
    ...
    //    Post second = Post.of(null, "Hello Hibernate Reactive", "My second Hibernate Reactive",
    // null);
    //
    //    sessionFactory
    //        .withTransaction(
    //            (conn, tx) ->
    //                conn.createQuery("DELETE FROM Post")
    //                    .executeUpdate()
    //                    .flatMap(r -> conn.persistAll(first, second))
    //                    .chain(conn::flush)
    //                    .flatMap(
    //                        r -> conn.createQuery("SELECT p from Post p",
    // Post.class).getResultList()))
    //        .subscribe()
    //        .with(
    //            data -> LOGGER.log(Level.INFO, "saved data:{0}", data),
    //            throwable -> LOGGER.warning("Data initialization is failed:" +
